...
# CHECK THIS CODE Problem_set_5_Number3_CiphertextMessage
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for i in range(26):
            shift = i
            self.encrypting_dict = super(CiphertextMessage, self).build_shift_dict(shift)
            self.message_text_encrypted = super(CiphertextMessage, self).apply_shift(shift)
            
            #check every element from the list, if lenght of them is equal to input
            #compare the values and add them to the tuple 
            for element in wordList:
                if len(element) == len(self.message_text):
                    if element == self.message_text_encrypted:
                        result_list.append(i)
                        result_list.append(element)
                        result_tuple = tuple(result_list)
                        print('Expected Output:',result_tuple)
                        return element
